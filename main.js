...
class Bambulab extends utils.Adapter {
    ...
    connectAllPrinters() {
        ...
        for (const printer of this.config.printers) {
            if (printer.enabled !== false) { // Default to enabled
        ...
        try {
            ...
            client.on('error', (error) => {
                if (!clientConnections[printer.serial].connectError) {
                    this.log.error(`MQTT connection error for printer ${printer.ip} (${printer.serial}): ${error.message}`);
    ...
    onMessage(obj) {
        ...
        switch (obj.command) {
            case '_addUpdatePrinter':
                this.handleAddUpdatePrinter(obj);
                break;
            case 'deletePrinter':
                this.handleDeletePrinter(obj);
                break;
            case 'loadPrinters':
                this.handleLoadPrinters(obj);
                break;
            default:
    ...
    /**
     * Handle add/update printer command from admin
     * @param {ioBroker.Message} obj - Message object
     */
    ...
    /**
     * Handle delete printer command from admin
     * @param {ioBroker.Message} obj - Message object
     */
    ...
    /**
     * Handle load printers command from admin
     * @param {ioBroker.Message} obj - Message object
     */
    handleLoadPrinters(obj) {
        try {
            const printersTable = [];
            ...
            if (this.config.printers) {
                for (const printer of this.config.printers) {
                    const connectionState = clientConnections[printer.serial] ? 
                        (clientConnections[printer.serial].connected ? 'Connected' : 'Disconnected') : 
                        'Disconnected';

                    const printState = currentPrintingStates[printer.serial] !== undefined ?
            ...
            this.log.error(`Error loading printers: ${error.message}`);
            this.sendTo(obj.from, obj.command, { error: error.message }, obj.callback);
        }
    }

    /**
     * Get print state text from numeric value
     * @param {number} state - Print state number
     * @returns {string} Human readable print state
     */
    getPrintStateText(state) {
        const states = {
            [-2]: 'Offline',
            [-1]: 'Idle', 
            [0]: 'Preparing',
